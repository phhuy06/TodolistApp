...
export default function Mainpage() {
    ...
    function handleAdd() {
        ...
        fetch("http://127.0.0.1:8000/create_work",{
            method:"POST",
    ...
    useEffect(()=>{
        const token = localStorage.getItem('token');
        const decode = jwt(token)
        fetch(`http://127.0.0.1:8000/list_works?day=${format(startDate,'yyyy-MM-dd')}&account=${decode.account}`,{
            method:"GET",
            headers: {
                "content-type": "application/json",
                Authorization: "Bearer ".concat(token)
    ...
    function handleRemovework(todo){
        ...
        fetch("http://127.0.0.1:8000/delete_works",{
            method:"DELETE",
            body: JSON.stringify(datasend),
            headers: {
                "content-type": "application/json",
